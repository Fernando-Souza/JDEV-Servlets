...
public class UsuarioDao {
    ...
    public UsuarioBean salvar(UsuarioBean usuario, Long usuarioLogado) {
        ...
        try {
            ...
            if (consultarUsuario(usuario.getLogin()) == null) {
                ...
                query = "insert into usuario (login,senha,nome, email,imagem,tipofile,useradmin,usuario_id,sexo,cep,rua,bairro,cidade,uf,numero,datanascimento,rendamensal) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
                ...
                insert.setDate(16, Date.valueOf(LocalDate.parse(usuario.getNascimento()).format(formatter)));
                insert.setDouble(17, usuario.getRendamensal());
    ...
    public void atualizar(UsuarioBean user) {
        ...
        String query = "update usuario set nome=?, email=?, imagem=?,tipofile=?,login=?, senha=?,"
                + "useradmin=?, sexo=?,cep=?,rua=?,bairro=?,cidade=?,uf=?,numero=?,datanascimento=?,rendamensal=? where id=?";
        ...
        try {
            ...
            statment.setDate(15, Date.valueOf(LocalDate.parse(user.getNascimento(), formatter)));
            statment.setDouble(16, user.getRendamensal());
            statment.setLong(17, user.getId());
    ...
    public List<UsuarioBean> listar(Long usuarioLogado, int offset, int n) {
        ...
        try {
            ...
            while (resultSet.next()) {
                ...
                usuario.setTelefones(daoTel.listaTelefone(usuarioLogado));
        ...
        try {
            ...
            while (resultSet.next()) {
                ...
                usuario.setNascimento(resultSet.getDate("datanascimento") == null ? null
                        : resultSet.getDate("datanascimento").toLocalDate());
                usuario.setRendamensal(resultSet.getDouble("rendamensal"));
                usuario.setTelefones(daoTel.listaTelefone(id));
    ...
    public List<UsuarioBean> listarTodos(Long usuarioLogado) {
        ...
        try {
            ...
            if (resultSet.next()) {
                ...
                usuario.setNascimento(resultSet.getDate("datanascimento") == null ? null
                        : resultSet.getDate("datanascimento").toLocalDate());
                usuario.setRendamensal(resultSet.getDouble("rendamensal"));
                usuario.setTelefones(daoTel.listaTelefone(userLogado));
            }
    ...
    public List<Telefone> listaTelefone(Long useId) {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                UsuarioBean usuariopai = this.consultarById(usuario_pai_id.toString(), usuario_cad_id);
                UsuarioBean usuariocad = this.consultarById(usuario_cad_id.toString());
